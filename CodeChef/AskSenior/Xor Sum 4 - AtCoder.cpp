...
int ways(int idx, int n, int k, bool flag, vector<int> &bit_ct, vector<vector<int>> &dp) {
    if (idx < 0) {
        return 0;
    }
    if (dp[idx][flag] != -1) {
    ...
    int temp2 = (1LL << idx), ct1 = bit_ct[idx], ct2 = n - ct1;
    for (int i = 0; i <= temp1; i++) {
        ...
        int sum2 = ways(idx - 1, n, k, flag && (i == temp1), bit_ct, dp);
        max_n = max(max_n, sum1 + sum2);
    }
    return dp[idx][flag] = max_n;
}
...
int32_t main() {
    ...
    for (int i = 60; i >= 0; i--) {
        int contrib = (1LL << i) % MOD;
        contrib = (contrib * v1[i]) % MOD;
        contrib = (contrib * (n - v1[i])) % MOD;
        sum = (sum + contrib) % MOD;
    }
    cout << sum << endl;
    return 0;
