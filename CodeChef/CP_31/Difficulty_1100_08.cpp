...
int32_t main() {
    ...
    cin >> t;
    while (t--) {
        int n, k, a, b, min_n1, min_n2 = LLONG_MAX, min_n3 = LLONG_MAX;
        cin >> n >> k >> a >> b;
        vector<int> v1(n), v2(n);
        for (int i = 0; i < n; i++) {
            cin >> v1[i] >> v2[i];
        }
        min_n1 = abs(v1[a - 1] - v1[b - 1]) + abs(v2[a - 1] - v2[b - 1]);
        for (int i = 0; i < k; i++) {
            min_n2 = min(min_n2, abs(v1[i] - v1[a - 1]) + abs(v2[i] - v2[a - 1]));
            min_n3 = min(min_n3, abs(v1[i] - v1[b - 1]) + abs(v2[i] - v2[b - 1]));
            min_n1 = min(min_n1, min_n2 + min_n3);
        }
        cout << min_n1 << endl;
    }
    return 0;
