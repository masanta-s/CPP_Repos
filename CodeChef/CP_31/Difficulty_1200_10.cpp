...
int32_t main() {
    ...
    while (t--) {
        ...
        sort(execution::par_unseq, v3.begin(), v3.end());
        for (int i = 0; i < n;) {
            if (v3[j].first == 0) {
                v4.emplace_back(v3[j].second, 0);
                j++;
            } else if (v1[i] > v3[j].first) {
                if (i > 0) {
                    v4.emplace_back(v3[j].second, v2[i - 1]);
                } else {
                    v4.emplace_back(v3[j].second, 0);
                }
                j++;
        ...
        if (v4.size() != v3.size()) {
            for (int i = v4.size(); i < v3.size(); i++) {
                v4.emplace_back(v3[i].second, v2.back());
            }
        }
