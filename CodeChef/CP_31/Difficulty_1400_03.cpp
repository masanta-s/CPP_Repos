...
int32_t main() {
    ...
    cin >> t;
    while (t--) {
        int n, max_n1 = INT_MIN, min_n1 = INT_MAX;
        cin >> n;
        vector<pair<int, int> > v1(n);
        for (int i = 0; i < n; i++) {
            cin >> v1[i].first >> v1[i].second;
            max_n1 = max({max_n1, v1[i].first, v1[i].second});
            min_n1 = min({min_n1, v1[i].first, v1[i].second});
        }
        int lo = min_n1, hi = max_n1;
        while (lo <= hi) {
            int mid = lo + ((hi - lo) >> 1);
            int min_n2 = 0, max_n2 = 0;
            bool flag = true;
            for (int i = 0; i < n; i++) {
                int l = v1[i].first, r = v1[i].second;
                min_n2 = max(l, min_n2 - mid);
                max_n2 = min(r, max_n2 + mid);
                if (min_n2 > max_n2) {
                    flag = false;
                    break;
                }
