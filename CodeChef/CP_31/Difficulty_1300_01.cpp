...
int32_t main() {
    ...
    cin >> t;
    while (t--) {
        int n, x, y, sum1 = 0, sum2 = 0, sum3 = 0;
        cin >> n >> x >> y;
        ...
        for (int i = 0; i < n; i++) {
            int el;
            cin >> el;
            v1.emplace_back(el % x, el % y);
            mp1[{el % x, el % y}]++;
        }
        for (int i = 0; i < n; i++) {
            if (v1[i].first == x - v1[i].first) {
                if (s1.find({v1[i].first, v1[i].second}) == s1.end()) {
                    int freq = mp1[{x - v1[i].first, v1[i].second}];
                    sum1 += ((freq * (freq - 1)) >> 1);
        ...
        for (int i = 0; i < n; i++) {
            if (!v1[i].first && s2.find({0, v1[i].second}) == s2.end()) {
                int freq = mp1[{0, v1[i].second}];
                sum3 += ((freq * (freq - 1)) >> 1);
                s2.insert({0, v1[i].second});
            }
        }
        cout << sum1 + (sum2 >> 1) + sum3 << endl;
    }
    return 0;
}
