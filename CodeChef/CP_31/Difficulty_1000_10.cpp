...
int32_t main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            mp1[str1[i]].push_back(i + n);
        }
        for (auto it1: mp1) {
            sort(execution::par_unseq, mp1[it1.first].begin(), mp1[it1.first].end());
        }
        for (int i = 0; i < mp1[ch].size(); i++) {
            auto it2 = upper_bound(mp1['g'].begin(), mp1['g'].end(), mp1[ch][i]);
            if (it2 != mp1['g'].end()) {
                max_n = max(max_n, *it2 - mp1[ch][i]);
            }
        }
        cout << max_n << endl;
    }
