...
int32_t main() {
    ...
    cin >> t;
    while (t--) {
        int n, sum1 = 0, sum2 = 0, min_n = INT_MAX, max_n1 = INT_MIN, max_n2 = INT_MIN;
        cin >> n;
        vector<int> v1(n);
        ...
        for (int i = 0; i < n; i++) {
            ...
            sum2 += v1[i];
            max_n2 = max(max_n2, sum2);
            if (max_n2 >= sum1 && !flag1 && i < n - 1) {
                flag2 = true;
                break;
            }
        }
        ...
        if ((min_n >= 0 || max_n2 < sum1) || (max_n2 == sum1 && !flag1)) {
            cout << "YES" << endl;
        } else {
            cout << "NO" << endl;
        }
