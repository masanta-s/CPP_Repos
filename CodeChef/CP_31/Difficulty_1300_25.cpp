...
int merge(vector<pair<int, int>> &arr, int st, int mid, int end) {
...
int mergeSort(vector<pair<int, int>> &arr, int st, int end) {
    if (st < end) {
        int mid = st + (end - st) / 2;
        int leftInv = mergeSort(arr, st, mid);
        int rightInv = mergeSort(arr, mid + 1, end);
        int currInv = merge(arr, st, mid, end);
