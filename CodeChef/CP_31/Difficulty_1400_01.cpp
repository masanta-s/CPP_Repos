...
int ways(int idx, int prev, vector<int> &v1, vector<int> &v2, vector<vector<int> > &dp, vector<vector<int> > &pref) {
    if (idx == v1.size()) {
        return 1;
    }
    if (dp[idx][prev] != -1) {
    ...
    int l = max(v1[idx], prev), r = v2[idx];
    if (l > r) {
        return dp[idx][prev] = 0;
    }
    if (pref[idx + 1][0] == -1) {
        pref[idx + 1][0] = ways(idx + 1, 0, v1, v2, dp, pref);
        for (int i = 1; i <= 3000; ++i) {
            pref[idx + 1][i] = (pref[idx + 1][i - 1] + ways(idx + 1, i, v1, v2, dp, pref)) % MOD;
        }
    }
    int sum = pref[idx + 1][r];
    if (l > 0) {
        sum = (sum - pref[idx + 1][l - 1] + MOD) % MOD;
    }
    return dp[idx][prev] = sum;
}

int count_trailing_zeros(string str1) {
    size_t last_nonzero = str1.find_last_not_of('0');
    if (last_nonzero == string::npos) {
        return str1.size();
    } else {
        return str1.size() - 1 - last_nonzero;
    }
}
...
int32_t main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            cin >> str1;
            sum1 += str1.size();
            int ct1 = count_trailing_zeros(str1);
            if (ct1) {
                v1.push_back(ct1);
            }
        }
