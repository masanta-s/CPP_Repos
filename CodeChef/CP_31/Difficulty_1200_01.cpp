...
int32_t main() {
    ...
    while (t--) {
        ...
        sort(execution::unseq, v1.begin(), v1.end(), greater<>());
        sort(execution::unseq, v2.begin(), v2.end(), greater<>());
        sort(execution::unseq, v3.begin(), v3.end(), greater<>());
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    if ((v1[i].second != v2[j].second) && (v1[i].second != v3[k].second) && (
                            v2[j].second != v3[k].second)) {
                        max_n = max(max_n, v1[i].first + v2[j].first + v3[k].first);
                    }
                }
            }
        }
